	...
	<view>
		...
		<view class="picker-content" :class="{'pickerShow':pickerShow}">
			<view class="picker-button">
				<text @click="closePicker">{{i18n.index.Cancel}}</text>
				<text @click="confirm">{{i18n.index.OK}}</text>
			</view>
			<!-- 单列选择 -->
			<picker-view class="picker-view" indicator-class="picker-view-selected-one" :value="pickerIndex" @change="pickerViewChangeOne"
			 v-if="mode=='one'">
				<picker-view-column>
					<view class="picker-item" v-for="(item, index) in pickerList" :key="index">{{i18n.index[item.label]}}</view>
				</picker-view-column>
			</picker-view>
			<!-- 双列选择-联动 -->
